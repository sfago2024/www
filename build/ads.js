function displayAdAndLogPageview() {
  ...
  document.querySelectorAll(".ad.desktop img").forEach((img) => { img.src = `/ads/${ad.name}-desktop.${ad.extension}`; });
  document.querySelectorAll(".ad.mobile img").forEach((img) => { img.src = `/ads/${ad.name}-mobile.${ad.extension}`; });
